# Balance Test Matrix

## Smoke

- Mod appears as "Civ VII: Equal Footing" in Additional Content.
- New game starts with base game only.
- New game starts with all owned DLC enabled.
- No database errors in the modding logs.

## Multiplayer

- 4-player Pangaea, Standard speed.
- 6-player Continents, Standard speed.
- 8-player Pangaea, Online speed.

## Checks

- Leader abilities still appear and trigger.
- Tooltip text mentions Equal Footing where changed.
- Mementos still equip unless the lobby manually disables them.
- No victory path becomes obviously noncompetitive.
- No start has direct adjacent natural wonder unless all starts are similarly strong.
